...
def demo_profile_markdown(
    ...
    common = [
        f"# RAGOps {profile} demo view: {decision}",
        "",
        "Evidence classification: `synthetic`.",
        "This credential-free benchmark does not demonstrate live deployment or customer usage.",
        "",
    ]
    if profile == "executive":
        common.extend(
            [
                "## Release risk",
                "",
                "The candidate loses required citations and grounded answers while latency and cost remain acceptable.",
                "",
                "## Required action",
                "",
                "Hold release, repair the affected retrieval/answer path, then rerun the same evidence contract.",
            ]
        )
        ...
        common.extend(["", "## Blocking gates", ""])
        common.extend(f"- `{gate}`" for gate in report.failed_gates)
    elif profile == "auditor":
        common.extend(["## Evidence manifest", ""])
        common.extend(
            f"- `{item['logical_name']}`: `{item['sha256']}`"
            for item in evidence_manifest["artifacts"]
        )
        common.extend(["", "## Declared limitations", ""])
        common.extend(f"- {item}" for item in evidence_manifest["limitations"])
    else:
